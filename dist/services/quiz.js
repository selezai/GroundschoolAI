...
class QuizService {
    ...
    async startQuiz(questionSetId, userId) {
        try {
            // Fetch questions from the API
            this.questionSet = await questionApi_1.default.getQuestionSet(questionSetId);
    ...
    async completeQuiz() {
        ...
        await questionApi_1.default.saveQuizResult(result);
        // Clear current session
        this.resetSession();
        return result;
    }
    async generateRecommendations(weakAreas) {
    ...
    getAnalytics() {
        ...
        return {
            averageTimePerQuestion: answers.reduce((sum, a) => sum + a.timeSpentSeconds, 0) / answers.length,
    ...
    calculateConfidenceScore() {
        ...
        const averageTime = answers.reduce((sum, a) => sum + a.timeSpentSeconds, 0) / answers.length;
    ...
    calculateTimeManagementScore() {
        ...
        const answers = this.currentSession.answers;
        const averageTime = answers.reduce((sum, a) => sum + a.timeSpentSeconds, 0) / answers.length;
        const timeVariance = answers.reduce((sum, a) => sum + Math.pow(a.timeSpentSeconds - averageTime, 2), 0) / answers.length;
